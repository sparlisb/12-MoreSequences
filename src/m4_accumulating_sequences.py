...
def rectangles_from_circles(circles):
    ...
    for k in range(len(circles)):
        c = circles[k]
        c_center = c.center
        c_l = rg.Point(c_center.x - c.radius, c_center.y - c.radius)
        c_r = rg.Point(c_center.x + c.radius, c_center.y + c.radius)
        rect = rect + [rg.Rectangle(c_l, c_r)]
    return rect
